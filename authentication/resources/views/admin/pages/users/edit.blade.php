                                    ...
                                    <div class="form-material">
                                        <select class="js-select2 form-control" id="type"
                                            ...
                                            @foreach(config('project.user_type') as $index=>$user_type)
                                                <option
                                                    value="0" {{old('type')?((old('type')==$index)?'selected':''):($user->type==$index?"selected":'')}}>
                                                    {{$user_type}}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <div class="form-material">
                                            <select class="js-select2 form-control" id="roles" name="roles[]" multiple
                                                ...
                                                @foreach($roles as $role)
                                                    <option
                                                        value="{{$role->id}}" {{old('roles')?(in_array($role->id,old('roles'))?'selected':''):(in_array($role->id,$user_roles)?'selected':'')}}>{{$role->display_name}}</option>
                                                @endforeach
                                            </select>
                                                ...
                                                @foreach($permissions as $index=>$permissionGroup)
                                                    <optgroup label="{{$index}}">
                                                        @foreach($permissionGroup as $permission)
                                                            <option
                                                                value="{{$permission->id}}" {{old('permissions')?(in_array($permission->id,old('permissions'))?'selected':''):(in_array($permission->id,$user_permissions)?'selected':'')}}>{{$permission->display_name}}</option>
                                                        @endforeach
                                                    </optgroup>
                                                @endforeach
                                            </select>
